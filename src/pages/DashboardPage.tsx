...
export default function DashboardPage() {
  ...
  // FIX: explicitly return object with all keys so TypeScript keeps them.
  // Using spread { ...buckets } was flattening to Record<string, number>
  // and losing named keys.
  const assetStats = useMemo(() => {
    ...
    assets.forEach((a) => {
      const k = a.status as keyof typeof buckets;
    ...
    return {
      ...
      'under-repair': buckets['under-repair'],
    ...
    <div className="space-y-6">
      ...
      <section>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard label="Total Products" value={productStats.total} icon={Package} color="pastel-blue" link="/products" delay={0} />
          <StatCard label="Available Stock" value={productStats.totalStock} icon={PackageCheck} color="pastel-green" link="/products" delay={0.05} />
          <StatCard label="Low Stock Items" value={productStats.low} icon={AlertTriangle} color="pastel-peach" link="/products" warning delay={0.1} />
          <StatCard label="Out of Stock" value={productStats.out} icon={PackageX} color="pastel-pink" link="/products" warning delay={0.15} />
        </div>
      </section>
      ...
      <section>
        <div className="flex items-center justify-between mb-3">
          ...
          <p className="text-xs text-brand-choco-soft">
            Total value:{' '}
            <span className="font-bold text-brand-orange">{formatINR(assetStats.totalValue)}</span>
          </p>
        </div>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard label="Total Assets" value={assetStats.total} icon={Laptop} color="pastel-blue" link="/assets" delay={0} />
          <StatCard label="Assigned" value={assetStats.assigned} icon={UserCheck} color="pastel-green" link="/assignments" delay={0.05} />
          <StatCard label="Under Repair" value={assetStats['under-repair']} icon={Wrench} color="pastel-peach" link="/assets" delay={0.1} />
          <StatCard label="Damaged" value={assetStats.damaged} icon={AlertTriangle} color="pastel-pink" link="/assets" delay={0.15} />
        </div>
      </section>

      <section>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <MiniStat label="Team Members" value={employees.length} icon={Users} trend={employees.filter((e) => e.status === 'active').length} trendLabel="active" link="/employees" />
          <MiniStat label="Stock In (7 days)" value={weeklyTotals.inTotal} icon={TrendingUp} trend={weeklyTotals.inTotal} trendLabel="units" positive link="/stock" />
          <MiniStat label="Stock Out (7 days)" value={weeklyTotals.outTotal} icon={TrendingDown} trend={weeklyTotals.outTotal} trendLabel="units" link="/stock" />
        </div>
      </section>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="card lg:col-span-2">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-brand-choco-soft mt-1">Last 7 days · In vs Out</p>
            </div>
          ...
          {pLoading ? (
            <div className="h-[280px] flex items-center justify-center text-brand-choco-soft">Loading...</div>
          ) : (
            <ResponsiveContainer width="100%" height={280}>
              <AreaChart data={stockMovementData}>
                ...
                <XAxis dataKey="day" stroke="#8B6F5A" fontSize={12} axisLine={false} tickLine={false} />
                <YAxis stroke="#8B6F5A" fontSize={12} axisLine={false} tickLine={false} />
                <Tooltip contentStyle={{ background: '#FFF8F0', border: '1.5px solid rgba(61, 40, 23, 0.1)', borderRadius: '16px', fontWeight: 500 }} />
                <Area type="monotone" dataKey="in" stroke="#F97316" strokeWidth={3} fill="url(#gIn)" />
                <Area type="monotone" dataKey="out" stroke="#8B6F5A" strokeWidth={3} fill="url(#gOut)" />
              </AreaChart>
            </ResponsiveContainer>
          )}
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} className="card">
          ...
          <p className="text-sm text-brand-choco-soft mt-1 mb-4">Current distribution</p>
          {assetStatusData.length === 0 ? (
            <div className="h-[200px] flex items-center justify-center text-sm text-brand-choco-soft">No assets yet</div>
          ) : (
            <ResponsiveContainer width="100%" height={200}>
              <PieChart>
                <Pie data={assetStatusData} cx="50%" cy="50%" innerRadius={55} outerRadius={85} paddingAngle={4} dataKey="value">
                  {assetStatusData.map((entry) => (<Cell key={entry.name} fill={entry.color} />))}
                </Pie>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="card lg:col-span-2">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h3 className="font-display text-2xl font-bold">Stock by Category</h3>
              <p className="text-sm text-brand-choco-soft mt-1">Current levels across categories</p>
            </div>
          </div>
          {categoryStockData.length === 0 ? (
            <div className="h-[260px] flex items-center justify-center text-sm text-brand-choco-soft">Add products to see category breakdown</div>
          ) : (
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={categoryStockData}>
                ...
                <CartesianGrid strokeDasharray="3 3" stroke="#3D28170F" />
                <XAxis dataKey="category" stroke="#8B6F5A" fontSize={12} axisLine={false} tickLine={false} />
                <YAxis stroke="#8B6F5A" fontSize={12} axisLine={false} tickLine={false} />
                <Tooltip cursor={{ fill: 'rgba(249, 115, 22, 0.05)' }} contentStyle={{ background: '#FFF8F0', border: '1.5px solid rgba(61, 40, 23, 0.1)', borderRadius: '16px' }} />
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }} className="card">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/audit" className="text-xs font-bold text-brand-orange flex items-center gap-1 hover:underline">
          ...
          {recentActivity.length === 0 ? (
            ...
            <div className="space-y-2">
              {recentActivity.map((a, i) => (
                <Link key={i} to={a.link} className="flex items-start gap-3 p-3 rounded-2xl hover:bg-brand-cream-dark transition">
                  <div className={cn(
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-brand-choco-soft mt-0.5">{formatDateTime(a.time)}</p>
                  </div>
                </Link>
              ))}
            </div>
...
function StatCard({
  label, value, icon: Icon, color, warning, delay = 0, link,
}: {
  label: string; value: number; icon: React.ComponentType<{ className?: string }>;
  color: string; warning?: boolean; delay?: number; link?: string;
}) {
  const content = (
    <motion.div
...
function MiniStat({
  label, value, icon: Icon, trend, trendLabel, positive, link,
}: {
  label: string; value: number; icon: React.ComponentType<{ className?: string }>;
  trend: number; trendLabel: string; positive?: boolean; link?: string;
}) {
  const inner = (
    <div className="card !p-5 flex items-center justify-between cursor-pointer hover:shadow-lift transition">
      <div>
        <p className="text-xs font-bold uppercase text-brand-choco-soft">{label}</p>
        <p className="font-display text-3xl font-bold mt-1">{value.toLocaleString('en-IN')}</p>
        <p className={cn('text-xs font-semibold mt-1', positive ? 'text-green-700' : 'text-brand-choco-soft')}>
